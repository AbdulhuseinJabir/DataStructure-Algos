...
class ListUsingArray implements ListInterface {
  ...
  // add item at position index, shifting all current items from
  // index onwards to the right by 1 
  // pre: 0 <= index <= size()
  ...
  public void insert(int index, int item) {
    if (num_items+1 > capacity) // array is full, enlarge it
      enlargeArr();
    // shift all items from last item to item at index to the 
    //right to create a gap
  ...
  public int remove(int index) {
    ...
    // shift all item from index+1 onwards to the left to 
    // close the gap
